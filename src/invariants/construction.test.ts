...
/** Catalog kinds plus this checkout's v0 top-level names until engines live under src/engine/. */
...
describe("construction", () => {
  ...
  test("INV-C-ENG-02: engine source does not import store, corpus, session, or cli", async () => {
    ...
    for (const file of files) {
      ...
      expect(file.text).not.toMatch(/from ["'][^"']*\/store/)
      expect(file.text).not.toMatch(/from ["'][^"']*\/corpus/)
