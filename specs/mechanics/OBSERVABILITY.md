...
**A standard "engine calls" section, not only a debug toggle.** `trace` narrates calls inline for debugging. Separately, every session should *close* with a short receipt: the `retention` calls that ran, one-line digest each — not to debug, but as assurance the tool drove the session and the questions weren't freelanced. **Decided 2026-09-11:** the retention-session skill adds a `receipt` config field, **default on**, printed after the Step 6 summary; `trace` stays a separate inline debug mode, default off. Still open: whether this should also exist as a real CLI affordance (`retention session --explain`) rather than only skill-side.
